...
private autoRun AutoState1 = autoRun.INIT;
    ...
    private autoMech AutoMechanics;
    
    private double PrevTime = Timer.getFPGATimestamp() + 0.2;
    
    public NewAutoBall5(MechMaster mechMaster, TorDrive torDrive){
        this.torDrive = torDrive;
        this.mechMaster = mechMaster;
        //linear1 = new linearTrajectory2(torDrive, 3.5f, 1.5);
        pivot1 = new pivotTrajectory(torDrive, 180, 1.5);
        linear2 = new linearTrajectory2(torDrive, 15.0f, 3.5);
    ...
    public void run(){
        ...
        switch(AutoState1){
            ...
            case PIVOT3:
                pivot3.run();
                mechMaster.autoRun(autoMech.STORE, turretMech.SET, 150);//previously set to 0
                if (pivot3.isDone()) {
                    torDrive.setMotorSpeeds(0, 0);
                    linear4.init();
                    PrevTime = Timer.getFPGATimestamp();
